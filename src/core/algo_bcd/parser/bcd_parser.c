 ...
 * bcd_parser.c
 *
 * Parses and validates a BCD compute request JSON string into an input_environment_t.
 * Contains all parsing helpers: float/bool/point/polygon parsing, winding detection,
 * edge building, and the main parse+validate entry point.
 *
 * Included by: bcd.c (as amalgamation)
 * Dependencies: bcd.h, bcd_check.h, cJSON, allocator
 */
