...
#define CSTAR_LINE_LAYER(layer_id, compute_layer_str, layer_name_str, z_index_str, edge_color_str, edge_style_str, visible_str, arrows_enabled_str)                  \
    {                                                                                                                                                                \
        cJSON *layer = cJSON_CreateObject();                                                                                                                         \
        cJSON_AddNumberToObject(layer, "id", (layer_id));                                                                                                            \
        cJSON_AddStringToObject(layer, "computeLayer", (compute_layer_str));                                                                                         \
        ...
        if ((arrows_enabled_str) && strcmp((arrows_enabled_str), "true") == 0)                                                                                       \
        {                                                                                                                                                            \
...
char *cstar_build_metadata_json(void)
{
    ...
    CSTAR_LINE_LAYER(1, "coveragePathPlan.coverage", "Coverage", "100", "#60a5fa", "solid", "true", "true")

    /* id=2  Coverage Transit — start→first node, last node→end */
    CSTAR_LINE_LAYER(2, "coveragePathPlan.coverageTransit", "Coverage Transit", "92", "#60a5fa", "dashed", "true", "true")

    /* id=3  Retreat Transit — dead-end A* escape path */
    CSTAR_LINE_LAYER(3, "coveragePathPlan.retreatTransit", "Retreat Transit", "88", "#f87171", "dashed", "true", "true")

    /* id=4  Hole Coverage — TSP loop inside a detected coverage hole */
    CSTAR_LINE_LAYER(4, "coveragePathPlan.holeCoverage", "Hole Coverage", "96", "#a78bfa", "solid", "true", "true")

    /* id=5  Hole Transit — move to/from hole subproblem entry/exit node */
    CSTAR_LINE_LAYER(5, "coveragePathPlan.holeTransit", "Hole Transit", "90", "#a78bfa", "dashed", "true", "true")
    ...
    CSTAR_POINT_LAYER(11, "rcgEndNodeList", "RCG End Nodes", "111", "#f97316", "#3f1d0a", "5", "#f97316", "false")

    /* id=12  RCG edges (Line) */

    CSTAR_LINE_LAYER(12, "rcgEdgeList", "RCG Edges", "108", "#334155", "solid", "false", "false")
